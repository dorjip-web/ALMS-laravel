        ...
        <form method="POST" action="{{ route('admin.users.create.store', [], false) }}">
                    ...
                    <select name="department_id" class="form-control" required>
                        @foreach ($departments ?? [] as $d)
                            <option value="{{ $d->department_id }}"
                                {{ old('department_id') == $d->department_id ? 'selected' : '' }}>{{ $d->department_name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div style="margin-top:12px">
                ...
                <a href="{{ route('admin.users.index') }}" class="btn"
                    style="margin-left:12px;background:#ddd;color:#333;border:1px solid #d3d3d3;padding:10px 18px;text-decoration:none;">Cancel</a>
            </div>
        </form>
    </div>
@endsection
